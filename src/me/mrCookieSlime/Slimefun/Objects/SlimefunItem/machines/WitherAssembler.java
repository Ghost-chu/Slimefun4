...
public class WitherAssembler extends SlimefunItem {
			...
			public void newInstance(final BlockMenu menu, final Block b) {
				try {
					if (!BlockStorage.hasBlockInfo(b) || BlockStorage.getLocationInfo(b.getLocation(), "enabled") == null || BlockStorage.getLocationInfo(b.getLocation(), "enabled").equals("false")) {
						menu.replaceExistingItem(22, new CustomItem(new ItemStack(Material.GUNPOWDER), "&7启用: &4\u2718", "", "&e> 点击来启用机器"));
						...
						menu.replaceExistingItem(22, new CustomItem(new ItemStack(Material.REDSTONE), "&7启用: &2\u2714", "", "&e> 点击来禁用机器"));
						menu.addMenuClickHandler(22, (p, slot, item, action) -> {
					...
					menu.replaceExistingItem(31, new CustomItem(new ItemStack(Material.PISTON), "&7偏移: &3" + offset + " 方块", "", "&r左键单击: &7+0.1", "&r右键单击: &7-0.1"));
	...
	private void constructMenu(BlockMenuPreset preset) {
		...
		preset.addItem(1, new CustomItem(new ItemStack(Material.WITHER_SKELETON_SKULL, (byte) 1), "&7凋零骷髅头颅槽", "", "&r此槽位接受凋灵骷髅头颅"),
			(p, slot, item, action) -> false
		);
		
		preset.addItem(7, new CustomItem(new ItemStack(Material.SOUL_SAND), "&7灵魂沙槽", "", "&r此槽位接受灵魂沙"),
			(p, slot, item, action) -> false
		);
		
		preset.addItem(13, new CustomItem(new ItemStack(Material.CLOCK), "&7冷却: &b30 秒", "", "&r机器最多需要冷却30秒", "&r所以...给它时间！"),
			(p, slot, item, action) -> false
		);
	}
